...
func TestExchangeAndRefresh(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case 1:
			...
			_, _ = io.WriteString(w, `{"access_token":"access-1","refresh_token":"refresh-1","expires_in":3600,"scope":"one two","token_type":"Bearer"}`)
		case 2:
	...
	cfg := Config{ClientID: "id", ClientSecret: "secret", TokenURL: server.URL, HTTPClient: server.Client(), Now: func() time.Time { return now }}
	token, err := Exchange(context.Background(), cfg, "code", "http://127.0.0.1/callback", "verifier")
	if err != nil {
		t.Fatal(err)
	}
	if token.AccessToken != "access-1" || token.RefreshToken != "refresh-1" || len(token.Scopes) != 2 || !token.Expiry.Equal(now.Add(time.Hour)) {
